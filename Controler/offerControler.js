...
const offerToWallet = asyncHandler(async (req, res) => {
     ...
     try {
          ...
          if (wallet) {
               ...
               await referalCollection.updateOne({ user: req.session.user, "invited.email":req.session.user.email }, {
                $set:{
                    "invited.$.status":true
                }
               });
...
const deleteCategoryOffer = asyncHandler(async(req,res)=>{
     ...
     await categoryCollection.updateOne({category:req.query.category},{
          $set:{
               discount:0
          }
     })
     ...
     await productCollection.updateMany({category:req.query.category},{
          $set:{
               categoryDiscount:0
          }
     });
     ...
     await productCollection.updateMany({catgory:req.query.catgory},{
          $inc:{
